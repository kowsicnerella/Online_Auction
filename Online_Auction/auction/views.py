...
def Signup_User(request):
    ...
    if request.method == 'POST':
        ...
        if 'registration_otp' in request.session:
            ...
            if entered_otp == request.session['registration_otp']:
            ...
            # OTP verification not in progress, generate and send OTP
            otp = generate_otp()
            send_otp_email(e, otp)

            # Store OTP and other registration details in the session for verification
            request.session['registration_otp'] = otp
            request.session['registration_details'] = {
                'fname': f,
                'lname': l,
                'uname': u,
                'email': e,
    ...
    d = {'error': error, 'otp_verified': otp_verified}
...
def result(request):
    ...
    d = {'error': error, 'people': people, 'data': pro, 'count': count, 'new2': new2}
...
def order_payment(request):
    user = User.objects.get(username=request.user.username)
    participent = Participant.objects.get(user=user)
    name = participent.user
    amount = participent.new_price
